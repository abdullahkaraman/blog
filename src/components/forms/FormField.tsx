		...
		switch (field.type) {
			case 'text':
				return (
					<Input
						...
						className="rounded-none border-0 border-b border-neutral-200 bg-white px-0 py-2 text-neutral-900 shadow-none placeholder:text-neutral-400 focus:border-black focus:ring-0"
			...
			case 'textarea':
				return (
					<Textarea
						placeholder={field.placeholder || ''}
						className="min-h-28 rounded-none border-0 border-b border-neutral-200 bg-white px-0 py-2 text-neutral-900 shadow-none placeholder:text-neutral-400 focus:border-black focus:ring-0"
						{...form.register(field.name!)}
					/>
				);
			case 'checkbox':
				...
				render={({ field: formField }) => (
					<FormItem>
						<FormLabel
							htmlFor={field.name!}
							className={cn(
								'flex items-center justify-between text-sm font-medium text-neutral-700',
